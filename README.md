...
这里放「站点适配器插件」和「通用适配规则」，改完在软件界面里点「🧩 适配器管理 → 重新加载适配器」即可生效，**不需要重启服务、不需要重新打包**。

- `site_generic.py`：通用兜底适配器（**不要删除**，所有网站的兜底靠它）
- `site_kuaimsp.py`：kuaimsp 专用（列表/搜索页抽卡片，详情页取 CDN m3u8，路径式翻页）
- `site_18mh.py`：18mh（禁漫天堂）专用（列表/搜索页抽卡片，详情页取带鉴权 m3u8，自动收集备选地址）
- `site_91porna.py`：91porna 专用（卡片识别 + JS 混淆破解，直取真实 m3u8）
- `site_bilibili.py`：bilibili 专用（适配新版页面，多 P 识别）
